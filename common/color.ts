...
function grid(xSteps, ySteps, colors: [string, string, string?, string?]) {
  // hex to rgb to luv
  const x0y0 = toLuv(space.rgb.luv(parse(colors[0]).values));
  const x1y0 = toLuv(space.rgb.luv(parse(colors[1]).values));
  const x0y1 = toLuv(space.rgb.luv(parse(colors[2] || colors[0]).values));
  const x1y1 = toLuv(space.rgb.luv(parse(colors[3] || colors[1]).values));
